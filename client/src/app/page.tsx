          ...
          <div className={`flex flex-col items-center gap-12 fade-in ${glitchActive ? 'glitch-text' : ''}`}>
            ...
            <div className="text-center space-y-6">
              ...
              <div className="space-y-2 px-2">
                <p className="text-[10px] sm:text-xs md:text-sm tracking-[0.15em] sm:tracking-[0.3em] md:tracking-[0.5em] uppercase text-white/30">
                  Nothing witnessed.
                </p>
                <p className="text-[8px] sm:text-[10px] md:text-xs tracking-[0.1em] sm:tracking-[0.2em] md:tracking-[0.3em] uppercase text-white/15">
            ...
            {/* Action buttons */}
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full max-w-[260px] sm:max-w-md mx-auto">
              <button
                onClick={() => setView('create')}
                className="btn-primary flex-1 py-3 sm:py-4"
                id="create-room-btn"
              >
                <span className="block text-[10px] sm:text-xs tracking-[0.1em] sm:tracking-[0.3em]">[ CREATE ROOM ]</span>
              </button>
              <button
                ...
                id="join-room-btn"
              >
                <span className="block text-[10px] sm:text-xs tracking-[0.1em] sm:tracking-[0.3em]">[ JOIN ROOM ]</span>
              </button>
            </div>

            {/* Bottom info */}
            <div className="text-center space-y-3 mt-8">
              <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 text-white/15 text-[8px] sm:text-[10px] tracking-[0.1em] sm:tracking-[0.3em] uppercase px-4 max-w-[280px] sm:max-w-none mx-auto">
                <span className="whitespace-nowrap">E2E ENCRYPTED</span>
                <span className="text-white/10 hidden sm:inline">|</span>
                <span className="whitespace-nowrap">ZERO KNOWLEDGE</span>
                <span className="text-white/10 hidden sm:inline">|</span>
                <span className="whitespace-nowrap">SELF-DESTRUCT</span>
              </div>
              <p className="text-white/8 text-[8px] sm:text-[9px] tracking-[0.15em] sm:tracking-widest uppercase px-4">
